...
fn check_url(req: &IncomingRequest) -> anyhow::Result<()> {
    ...
    let expected = "127.0.0.1:3000";
    anyhow::ensure!(
        authority.as_deref() == Some(expected),
        "URL was expected to be '{expected}' but was '{authority:?}'"
    );

    let path_with_query = req.path_with_query();
    let expected = "/base/path?query=string";
    anyhow::ensure!(
...
fn check_headers(req: &IncomingRequest) -> anyhow::Result<()> {
    ...
    let headers = [
        ...
        ("spin-full-url", "http://example.com/base/path?query=string"),
        ("spin-path-info", "/path"),
        ("spin-base-path", "/base"),
...
fn header_as_string<'a>(req: &'a IncomingRequest, name: &str) -> anyhow::Result<String> {
